...
Terrain::Terrain(size_t numRows, size_t numCols, size_t size, float minElevation,
    ...
    mCamera(camera)
{
    // Validation of inputs.  TODO: The port to GTL must replace these
    // with exception handling.
    LogAssert(mNumRows > 0 && mNumCols > 0, "Invalid number of rows or columns.");
    ...
    LogAssert(type == DF_R32G32B32_FLOAT || type == DF_R32G32B32A32_FLOAT, "VertexFormat type is not supported.");
    unsigned int offset = vformat.GetOffset(index);
    LogAssert(offset == 0, "VertexFormat offset must be 0.");
    LogAssert(mCamera, "Camera must exist.");
...
{
    std::shared_ptr<Visual> page;
    if (row < mNumRows && col < mNumCols)
    {
        auto child = mChild[col + mNumCols * row];
        page = std::dynamic_pointer_cast<Visual>(child);
    }
    else
    {
        LogError("Invalid input to GetPage.");
        page = nullptr;
    }
    return page;
}
...
{
    if (row < mNumRows && col < mNumCols && heights.size() >= mSize * mSize)
    {
        auto child = mChild[col + mNumCols * row];
    ...
    LogError("Invalid input to SetHeights.");
}
...
std::vector<unsigned short> const& Terrain::GetHeights(size_t row, size_t col) const
{
    ...
    {
        auto child = mChild[col + mNumCols * row];
        page = std::dynamic_pointer_cast<Page>(child);
    }
    else
    {
        // Assert on invalid inputs but in non-assertion mode, return the
        // child[0] heights to avoid having a static dummy height array
        // whose reference value would be returned otherwise.
        LogError("Invalid input to SetHeights.");
        page = std::dynamic_pointer_cast<Page>(mChild[0]);
    }
...
{
    // Create a mesh for the page.  The vertices are initialized using
    // the SetHeights(...) function.
    MeshFactory mf;
