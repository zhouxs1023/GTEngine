...
int main(int, char const*[])
{
    ...
    Window::Parameters parameters(L"IKControllersWindow", 0, 0, 512, 512);
    auto window = TheWindowSystem.Create<IKControllersWindow>(parameters);
    TheWindowSystem.MessagePump(window, TheWindowSystem.DEFAULT_ACTION);
    TheWindowSystem.Destroy<IKControllersWindow>(window);
    return 0;
}

IKControllersWindow::IKControllersWindow(Parameters& parameters)
    :
    Window3(parameters)
{
...
void IKControllersWindow::OnIdle()
{
    mTimer.Measure();
...
bool IKControllersWindow::OnCharPress(unsigned char key, int x, int y)
{
    if (Transform(key))
    {
        return true;
...
bool IKControllersWindow::OnMouseMotion(MouseButton button, int x, int y, unsigned int modifiers)
{
...
void IKControllersWindow::CreateScene()
{
    // Scene
    //     GroundMesh
    //     IKSystem
...
std::shared_ptr<Visual> IKControllersWindow::CreateCube()
{
...
std::shared_ptr<Visual> IKControllersWindow::CreateRod()
{
    VertexFormat vformat;
    vformat.Bind(VA_POSITION, DF_R32G32B32_FLOAT, 0);
    vformat.Bind(VA_COLOR, DF_R32G32B32A32_FLOAT, 0);
...
std::shared_ptr<Visual> IKControllersWindow::CreateGround()
{
    VertexFormat vformat;
    vformat.Bind(VA_POSITION, DF_R32G32B32_FLOAT, 0);
    vformat.Bind(VA_COLOR, DF_R32G32B32A32_FLOAT, 0);
...
void IKControllersWindow::UpdateRod()
{
...
bool IKControllersWindow::Transform(unsigned char key)
{
