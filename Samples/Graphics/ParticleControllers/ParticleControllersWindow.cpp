...
#include "ParticleControllersWindow.h"
...
int main(int, char const*[])
{
    ...
    Window::Parameters parameters(L"ParticleControllersWindow", 0, 0, 640, 480);
    auto window = TheWindowSystem.Create<ParticleControllersWindow>(parameters);
    TheWindowSystem.MessagePump(window, TheWindowSystem.DEFAULT_ACTION);
    TheWindowSystem.Destroy<ParticleControllersWindow>(window);
    return 0;
}

ParticleControllersWindow::ParticleControllersWindow(Parameters& parameters)
    :
    Window3(parameters)
{
...
void ParticleControllersWindow::OnIdle()
{
    mTimer.Measure();
...
bool ParticleControllersWindow::OnCharPress(unsigned char key, int x, int y)
{
    switch (key)
    {
    case 'w':
...
void ParticleControllersWindow::CreateScene()
{
