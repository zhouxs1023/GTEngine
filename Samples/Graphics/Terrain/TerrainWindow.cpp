...
int main(int, char const*[])
{
    ...
    Window::Parameters parameters(L"TerrainWindow", 0, 0, 1024, 768);
    auto window = TheWindowSystem.Create<TerrainWindow>(parameters);
    TheWindowSystem.MessagePump(window, TheWindowSystem.DEFAULT_ACTION);
    TheWindowSystem.Destroy<TerrainWindow>(window);
    return 0;
}

TerrainWindow::TerrainWindow(Parameters& parameters)
    :
    Window3(parameters),
...
bool TerrainWindow::OnKeyDown(int key, int, int)
{
    return mTerrainCameraRig.PushMotion(key);
}

bool TerrainWindow::OnKeyUp(int key, int, int)
{
    return mTerrainCameraRig.PopMotion(key);
}

bool TerrainWindow::SetEnvironment()
{
...
void TerrainWindow::CreateScene()
{
...
void TerrainWindow::CreateTerrain()
{
...
void TerrainWindow::CreateSkyDome()
{
...
void TerrainWindow::UpdateScene()
{
...
void TerrainWindow::TerrainCameraRig::Initialize(std::shared_ptr<Camera> const& camera,
...
void TerrainWindow::TerrainCameraRig::MoveForward()
{
...
void TerrainWindow::TerrainCameraRig::MoveBackward()
{
...
void TerrainWindow::TerrainCameraRig::MoveDown()
{
...
void TerrainWindow::TerrainCameraRig::MoveUp()
{
    mHeightAboveTerrain += GetTranslationSpeed();
    Vector4<float> camPosition = mCamera->GetPosition();
    float height = mTerrain->GetHeight(camPosition[0], camPosition[1]);
