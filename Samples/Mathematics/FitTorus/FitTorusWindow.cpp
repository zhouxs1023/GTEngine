...
int main(int, char const*[])
{
    ...
    Window::Parameters parameters(L"FitTorusWindow", 0, 0, 512, 512);
    auto window = TheWindowSystem.Create<FitTorusWindow>(parameters);
    TheWindowSystem.MessagePump(window, TheWindowSystem.DEFAULT_ACTION);
    TheWindowSystem.Destroy<FitTorusWindow>(window);
    return 0;
}

FitTorusWindow::FitTorusWindow(Parameters& parameters)
    :
    Window3(parameters)
{
...
bool FitTorusWindow::OnCharPress(unsigned char key, int x, int y)
{
    switch (key)
    {
    case 'w':
...
void FitTorusWindow::CreateScene()
{
...
void FitTorusWindow::CreatePoints(std::vector<Vector3<double>> const& X)
{
...
void FitTorusWindow::CreateGNTorus(std::vector<Vector3<double>> const& X,
...
void FitTorusWindow::CreateLMTorus(std::vector<Vector3<double>> const& X,
    Vector3<double>& C, Vector3<double>& N, double& r0, double& r1)
{
    ApprTorus3<double> fitter;
    size_t const maxIterations = 128;
...
std::shared_ptr<Visual> FitTorusWindow::CreateTorusMesh(
