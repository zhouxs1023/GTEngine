...
namespace gte
{
    ...
    bool GetContainer(int numPoints, Vector3<Real> const* points, OrientedBox3<Real>& box)
    {
        ...
        if (fitter.Fit(numPoints, points))
        {
            ...
            Vector3<Real> pmin{ Dot(diff, box.axis[0]), Dot(diff, box.axis[1]), Dot(diff, box.axis[2]) };
    ...
    // input boxes.
    template <typename Real>
    bool MergeContainers(OrientedBox3<Real> const& box0, OrientedBox3<Real> const& box1,
        OrientedBox3<Real>& merge)
    {
        // The first guess at the box center. This value will be updated later
        // after the input box vertices are projected onto axes determined by
        // an average of box axes.
        ...
        // endpoints on the axes are C+min[i]*D[i] and C+max[i]*D[i]. The
        // point C is not necessarily the midpoint for any of the intervals.
        // The actual box center will be adjusted from C to a point C' that is
