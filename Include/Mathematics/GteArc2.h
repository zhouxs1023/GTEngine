...
namespace gte
{
    // The circle containing the arc is represented as |X-C| = R where C is
    // the center and R is the radius. The arc is defined by two points end[0]
    // and end[1] on the circle so that end[1] is obtained from end[0] by
    // traversing counterclockwise. The application is responsible for
    // ensuring that end[0] and end[1] are on the circle and that they are
    // properly ordered.
    ...
    public:
        ...
        // center to (0,0), radius to 1, end[0] to (1,0) and end[1] to (0,1).
        Arc2()
            :
        ...
        // Test whether P is on the arc. The application must ensure that P is
        // on the circle; that is, |P-C| = R. This test works for any angle
        // between B-C and A-C, not just those between 0 and pi radians.
